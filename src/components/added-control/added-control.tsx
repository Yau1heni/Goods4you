...
export const AddedControl: FC<AddedControlProps> = (props) => {
    ...
    return (
        <div className={styles.addedControl}>
            <Button className={styles.button} disabled={quantityProducts === 0 || isLoading} onClick={decreaseQuantityHandler}>
                <Decrease/>
            </Button>
            <div className={styles.textContainer}>
                <p className={styles.text}>{quantityProducts} {itemsText}</p>
            </div>
            <Button disabled={stock === quantityProducts || isLoading} className={styles.button} onClick={increaseQuantityHandler}>
                <Increase/>
            </Button>
        </div>
    );
